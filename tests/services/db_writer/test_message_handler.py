...
from components.db_service.message_handler import consume_save_crawl_data, consume_save_parsed_data
...
class TestMessageHandlers(unittest.TestCase):
    ...
    @patch('components.db_service.message_handler.save_crawled_page')
    def test_handle_save_page_message_success(self, mock_save):
        body = json.dumps({"url": "http://example.com"}).encode()
        consume_save_crawl_data(self.ch, self.method,
    ...
    @patch('components.db_service.message_handler.save_crawled_page', side_effect=Exception("DB error"))
    ...
    @patch('components.db_service.message_handler.save_parsed_page_content', side_effect=Exception("Some failure"))
