...
def test_add_to_visited_set(cache_service, mock_redis, mock_logger):
    assert cache_service.add_to_visited_set("http://test.com") is True
    mock_redis.sadd.assert_called_with(
        RedisSets.VISITED.value, "http://test.com")
    mock_logger.info.assert_called()


def test_add_to_visited_set_none(cache_service):
    assert cache_service.add_to_visited_set(None) is False
...
def test_is_in_visited(cache_service, mock_redis):
    mock_redis.sismember.return_value = True
    assert cache_service.is_in_visited("http://test.com") is True
    mock_redis.sismember.assert_called_with(
        RedisSets.VISITED.value, "http://test.com")


def test_is_in_enqueued(cache_service, mock_redis):
    mock_redis.sismember.return_value = True
